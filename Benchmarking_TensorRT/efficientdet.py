...
sizeschange=[[8,8], [16,16], [32,32], [64,64],[8,8], [16,16], [32,32], [64,64],[8,8], [16,16], [32,32], [64,64] ]############sanity check for sizes while not using scale_factor in interpolate#########
...
class Interpolate2d(nn.Module):
    ...
    def __init__(self,
        ...
        self.size = size
##################this is agian commented for onnx conversion purpose########################
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        #out_a = F.interpolate(input, self.scale_factor, mode='bilinear', align_corners=False)
        s = input.size(2) * 2, input.size(3) * 2 ####2 is the scale_factor
        self.size=s
        out_a = F.interpolate(input, self.size, mode='bilinear', align_corners=False) #here I am not using scaling factor
        return out_a
    ...
    def __init__(
        ...
        if reduction_ratio > 1:
            ...
            if downsample in ('max', 'avg'):
                ...
                '''if iter < 12:
                    downsample = Interpolate2d(size=sizeschange[iter] , mode=downsample)
                iter+=1'''
            ...
            if reduction_ratio < 1:
                ...
                #############so this is only done to export to onnx###########

                self.add_module('upsample', Interpolate2d(mode=upsample))
                #self.add_module('upsample', Interpolate2d(scale_factor=scale, mode=upsample))
                '''if iter <12:
                    self.add_module('upsample', Interpolate2d(size=sizeschange[iter],mode=upsample))
                iter+=1'''
...
class EfficientDet(nn.Module):
    ...
    def __init__(self, config, pretrained_backbone=True, alternate_init=False):
        ...
        self.backbone = create_model(
            ...
            pretrained=pretrained_backbone, exportable=True, **config.backbone_args) #erase scriptable=True
